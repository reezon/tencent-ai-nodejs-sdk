 ...
 */
exports.ocrIdCard = async function (image, cardType = 0) {
 ...
 * 车牌OCR
 * 识别车牌上面的字段信息
 * 详情请见：<https://ai.qq.com/doc/plateocr.shtml>
 * @param {String} image 原始图片的base64编码数据（原图大小上限1MB，支持JPG、PNG、BMP格式）
 * @param {String} imageUrl image和image_url都提供，仅支持image_url，image和image_url必须至少提供一个
 */
exports.ocrPlate = async function (image, imageUrl) {
  const apiUrl = this.prefix + '/ocr/ocr_plateocr'
 ...
 * 手写体OCR
 * 检测和识别图像上面手写体的字段信息
 * 详情请见：<https://ai.qq.com/doc/handwritingocr.shtml>
 * @param {String} image 原始图片的base64编码数据（原图大小上限1MB，支持JPG、PNG、BMP格式）
 * @param {String} imageUrl 如果image和image_url都提供，仅支持image_url，image和image_url必须至少提供一个
 */
exports.ocrHandWriting = async function (image, imageUrl) {
  const apiUrl = this.prefix + '/ocr/ocr_handwritingocr'
